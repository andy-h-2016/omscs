...
class Switch(StpSwitch):
    ...
    def update_root(self, message):
        ...
        Update if incoming message has a lower claimed root
        Update distance if root is updated
        Update distance if shorter path to the same root is updated.
        """
        ...
        currentActiveLinks = set(self.activeLinks)

        newDistance = message.distance + 1
        if message.root < currentRoot:
            ...
            if newDistance == currentDistance and message.origin < self.pathToRoot:
                ...
                self.activeLinks.remove(currentPathToRoot)
